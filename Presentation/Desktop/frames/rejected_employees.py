...
class RejectEmployeeFrame(Frame):
    def __init__(self, window, manager, employee_business):
        ...
        self.header_label = Label(self, text="----------- Reject Manager Form -----------")
        ...
        self.reject_treeview = Treeview(self, columns=(
                                                        "first_name",
                                                        "last_name",
                                                        "national_code",
                                                        "username",
        ...
        self.reject_treeview.grid(row=2, column=0, columnspan=7, pady=(0, 10), padx=10, sticky="snew")
        self.reject_treeview.column("#0", width=50)
        self.reject_treeview.heading("#0", text="#")
        self.reject_treeview.heading("#1", text="First Name")
        self.reject_treeview.heading("#2", text="Last Name")
        self.reject_treeview.heading("#3", text="National Code")
        self.reject_treeview.heading("#4", text="User Name")
        self.reject_treeview.heading("#5", text="Status")
        self.reject_treeview.heading("#6", text="Reject Date")

        self.reject_treeview.column("#0",width=50,stretch=False)

        for col in self.reject_treeview["columns"]:
            self.reject_treeview.column(col, width=120, anchor="center")

        self.reject_treeview.bind("<Double-1>", self.doubel_clicked)
    ...
    def refresh_butten_clicked(self):
        self.data_load_to_reject_treeview()

    @PerformanceLogger
    def data_load_to_reject_treeview(self, page_number=1, page_size=15):
        self.page_size = page_size
        term = self.search_entry.get().strip()
        response = self.employee_business.get_reject_employee(page_number, page_size, term if term else None)

        if response.success:
            for row in self.reject_treeview.get_children():
                self.reject_treeview.delete(row)

            for index, employee in enumerate(response.data):
                rownumber = (page_number - 1) * page_size + index + 1
                self.reject_treeview.insert(
                    "",
                    "end",
                    iid=employee.id,
                    text=str(rownumber),
    ...
    def doubel_clicked(self, event):
        employee_id = self.reject_treeview.focus()
        if not employee_id:
            return
    ...
    def load_next_data_to_treeview(self):
        ...
        data = self.data_load_to_reject_treeview(next_page)
    ...
    def load_previous_data_to_treeview(self):
        ...
        self.data_load_to_reject_treeview(previous_page)
