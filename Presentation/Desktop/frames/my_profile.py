...
from ttkbootstrap.style import LIGHT,DANGER,SUCCESS,WARNING,INFO
...
class MyProfileFrame(Frame):
    def __init__(self,window,manager,employee_business:EmployeeBusinessLogic):
        ...
        self.header_label=Label(self,text="-------- My Profile --------")
    ...
    def save_profile_button_clicked(self):
        ...
        role_id = self.entry_employee_role.get()
        update_employee = self.employee_business.update_profile(employee_id,new_firstname,new_lastname,new_username
                                                                ,new_national_code,new_email,role_id)
    ...
    def butten_reset_password_clicked(self):
        self.manager.show_frame("reset password")


    def butten_arrow_clicked(self):
        if self.manager.current_user.role_id == EmployeeRole.Admin:
            self.manager.back()
            self.error_label.config(text="")
        else:
            self.manager.show_frame("Home")
            self.error_label.config(text="")

    @PerformanceLogger
    def set_entry(self):
        self.employee_id = self.manager.current_user.id
        response_image = self.employee_business.get_image_employee(self.employee_id)
        response = self.employee_business.get_employee_by_id(self.employee_id)
        ...
        if response.success:
            ...
            if response.data.status_id == EmployeeStatus.Pending:
